...
const ASCII_ONLY = !config.utf8 || true;
...
exports.setup = async () => {
  ...
  emitter.on( 'channels/broadcast', payload => {
    ...
    // filter out folks we don't want to hear from
    ...
    const msg = ASCII_ONLY ? clean.strip.nonASCII( payload.message ) : payload.message;
  ...
  logger.info( 'connected to gossip' );
...
exports.channels = {
  send : async ( name, channel, message ) => {
    logger.debug('OUT ' + channel + ' ' + name + ': ' + message );
