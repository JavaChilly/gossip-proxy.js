...
let RELOAD_CHECK_INTERVAL = 5000; // 5 seconds

let fetchData = function( path, callback ) {
...
let RELOADING = exports.metrics.reload_cache;
...
setInterval(function() {
  ...
  for (let path of Object.keys( RELOADING ) ) {
    let state = RELOADING[ path ];
    if ( state.triggered ) {
      ...
      fetchData( path, function( path, err, data ) {
        ...
        stateRef.lastLoaded = (new Date()).getTime();
        stateRef.func( err, data );
      });
    }
  }
